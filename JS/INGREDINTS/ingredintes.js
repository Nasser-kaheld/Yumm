...
async function Yummy() {
  ...
  if (x.ok) {
    ...
    display(data.categories); // نعرض الفئات
  } else {
...
function display(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    cartona += `
     <div class="col">
        <div class="inner icon-respiy text-center fs-4 text-white">
...
$(document).on('click', '.col', function () {
    const pText = $(this).find('p').first().text(); 
    console.log(pText);
    ...
    setTimeout(function () {
        window.location.href = `./ingredintsitem.html`;
    }, 500);
});
